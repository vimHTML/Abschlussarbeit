...
jQuery(document).ready(function(){
  ...
  /*try {
    ...
    if(navigator.mimeTypes ["application/x-shockwave-flash"] != undefined) hasFlash = true;
        ...
        // alert('FLASH available? ' + hasFlash);
  if(hasFlash){
      jQuery('#content #container').hide();
  }else{
            setTimeout("hideDynamic()", 500);
        }


  jQuery('#display .layer img').fadeTo(1, 0);
  getData();
  ...
  jQuery("#test").slider({
    ...
    change: function(event, ui) {
      //console.log('handling evt change....'); 
      if(parseInt(ui.value) % steps  == 0)
      {
        renderData(parseInt(ui.value) / steps);
      }else{
        factor = parseInt(ui.value) % steps;
        renderData((ui.value - factor)/steps, factor);
      }
    },
    slide: function(event, ui) {
      //console.log('handling evt slide....'); 
      if(parseInt(ui.value) % steps  == 0)
      {
        renderData(parseInt(ui.value) / steps);
      }else{
...
function getData(){
  jQuery.ajax({
    type: "GET",
    //url: 'http://www.forum-flughafen-region.de/fileadmin/files/unhtools/data.xml',
    url: 'http://freespace.square7.ch/data.xml',
    dataType: "xml",
    success: function(data) {
      ...
      parsedData.items = [];
       ...
       jQuery('p#info').html(jQuery(data).find('info').text());
       renderData(0);
       initGradient();
    }
...
function renderData(index, offset){
  curDate = null;
